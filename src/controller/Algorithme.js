...
exports.createAlgorithme =async(req, res) => {
    algorithmeToUpdate = await Algorithme.findOne({})
    const {name,introduction}= req.body;
    if (algorithmeToUpdate){
        if(name){
            algorithmeToUpdate.name = name
        }
        if(introduction){
            algorithmeToUpdate.introduction=introduction
        }
        let bg = []
        ...
        algorithmeToUpdate.bg=bg
        }
        algorithmeToUpdate.save((error, algorithme) => {
            if (algorithme) {
                return res.status(200).json({ message: "algorithme updated sucsessfully !" })
            }
