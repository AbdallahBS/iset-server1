...
exports.createMath = async(req, res) => {
    mathToUpdate = await Math.findOne({})
    const {name,introduction}= req.body;
    if (mathToUpdate){
        if(name){
            mathToUpdate.name = name
        }
        if(introduction){
            mathToUpdate.introduction=introduction
        }
        let bg = []
        ...
        mathToUpdate.save((error, math) => {
            if (math) {
                return res.status(200).json({ message: "math updated sucsessfully !" })
            }
