...
exports.createReseau = async(req, res) => {
    ...
    reseauToUpdate = await Reseau.findOne({})
    const {name,introduction}= req.body;
    if (reseauToUpdate){
        if(name){
            reseauToUpdate.name = name
        }
        if(introduction){
            reseauToUpdate.introduction=introduction
        }
        let bg = []
        ...
        reseauToUpdate.bg=bg
        }
        reseauToUpdate.save((error, reseau) => {
            if (reseau) {
                return res.status(200).json({ message: "reseau updated sucsessfully !" })
            }
